...
| A server that runs on any machine | A high-memory enterprise pipeline |
...
|---|---|---|---|
| Basic | `servers/data_basic/` | Load, inspect, filter, patch, restore | 8 tools |
| Medium | `servers/data_medium/` | Profiling, cleaning pipelines, aggregations | 22 tools |
| Advanced | `servers/data_advanced/` | Dashboard generation, visual export, profiling reports | 11 tools |
...
Pin Python to `3.14`. Use `uv` as the package manager. Never use `pip` or
...
| Var | Effect |
|---|---|
| `MCP_OUTPUT_DIR` | Default directory for generated files. Outranks the input file's directory; an explicit `output_path` argument still wins. |
...
using real generated datasets) is what `remote_smoke_test.sh` covers, run after `docker
