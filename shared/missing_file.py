...
`Ad_Data.xlsx` for a caller who remembered the wrong extension, is exactly the
answer it needed and the only thing that could not see it was the server.

Sixty-odd tools report a missing file, each with its own copy of that hint, so
this is a choke point on the response rather than sixty edits: any failure
