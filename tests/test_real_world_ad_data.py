"""Integration tests against a real (not synthetic) Ad_Data.csv sample.
...
See ad_data_real_sample_csv fixture in conftest.py and the project memory
...
class TestRealWorldCleaningPipeline:
    ...
    def test_apply_patch_real_pipeline(self, ad_data_real_sample_csv):
        r = apply_patch(
            str(ad_data_real_sample_csv),
            ops=[
        ...
        df = pd.read_csv(ad_data_real_sample_csv)
...
class TestRealWorldWorkspacePipeline:
    ...
    def test_run_workspace_pipeline_leaves_raw_untouched(self, ad_data_real_sample_csv, project_base):
        original_bytes = ad_data_real_sample_csv.read_bytes()
        create_workspace("real_ad_data", base_dir=str(project_base))
        register_workspace_file(
            "real_ad_data", str(ad_data_real_sample_csv), alias="raw", stage="raw", base_dir=str(project_base)
        )
        ops = [
            {"op": "fill_nulls", "column": "link_clicks", "strategy": "median"},
            {"op": "column_math", "formula": "clicks / impressions * 100", "target_column": "ctr_pct"},
        ...
        assert r["success"] is True
        assert ad_data_real_sample_csv.read_bytes() == original_bytes, "raw input must not be mutated"
