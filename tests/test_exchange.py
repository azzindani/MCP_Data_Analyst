...
def test_html_output_path_prefers_env_over_input_parent(monkeypatch, tmp_path):
    monkeypatch.setenv("MCP_OUTPUT_DIR", str(tmp_path / "shared"))
    out = get_output_path("", tmp_path / "raw" / "sales.csv", "eda")
    assert out == tmp_path / "shared" / "sales_eda.html"


def test_html_output_path_explicit_argument_still_wins(monkeypatch, tmp_path):
    monkeypatch.setenv("MCP_OUTPUT_DIR", str(tmp_path / "shared"))
    explicit = tmp_path / "custom" / "report.html"
    explicit.parent.mkdir()
    assert get_output_path(str(explicit), tmp_path / "sales.csv", "eda") == explicit
...
def test_embed_content_attaches_public_url_without_return_content(remote_mode):
    ...
    result = embed_content({"success": True, "op": "generate_chart"}, chart, False)
...
@pytest.mark.parametrize(
    "url",
    [
        "http://127.0.0.1:8810/basic/health",
...
def test_assert_fetchable_allows_private_when_opted_in(monkeypatch):
    monkeypatch.setenv("MCP_FETCH_ALLOW_PRIVATE", "1")
    assert_fetchable("http://127.0.0.1:8810/health")
