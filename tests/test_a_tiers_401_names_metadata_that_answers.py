...
NAMES = ("basic", "medium", "statistics", "transform", "visual", "workspace", "ingest")
...
out = {}
with TestClient(unified_server.app, base_url=os.environ["DA_PUBLIC_URL"]) as c:
...
def discovery(tmp_path_factory) -> dict:
    ...
    env = {**os.environ, "DA_API_KEY": "test-key-not-a-secret", "DA_PUBLIC_URL": PUBLIC}
    for name in (*NAMES, "domain"):
        env[f"DA_{name.upper().replace('-', '_')}_OAUTH_STATE_DIR"] = str(state / name)
