...
- **Quality breakdown**: `{completeness, validity, uniqueness, drift}` behind the 0–100 score. `validity` counts what breaks a rule (a constant column costs its share of the columns); distribution advice (zeros, skew, outliers, correlation, imbalance, cardinality) is listed and counted in `not_scored`, not scored. `drift` is reported as `None` rather than invented — it needs a baseline, and a component scored 100 because nothing was measured would be a different kind of lie. Pass `compare_to` and it becomes a number
